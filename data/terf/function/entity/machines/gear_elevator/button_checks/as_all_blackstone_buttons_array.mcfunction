...
data modify storage terf:temp args.command set value "function terf:entity/machines/gear_elevator/button_checks/as_all_blackstone_buttons"
function datapipes_lib:require/run_at with storage terf:temp args

data remove storage terf:temp args.array[0]
execute if data storage terf:temp args.array[0] run function terf:entity/machines/gear_elevator/button_checks/as_all_blackstone_buttons_array
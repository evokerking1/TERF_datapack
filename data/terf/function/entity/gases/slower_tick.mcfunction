...
execute if block ~ ~-1 ~ water run kill @s
execute if entity @s[tag=terf_fire] if predicate terf:flammable_nearby unless entity @e[type=bat,tag=terf_fire,distance=0.01..3,limit=1] unless predicate datapipes_lib:can_see_sky run function terf:entity/gases/fire_spread

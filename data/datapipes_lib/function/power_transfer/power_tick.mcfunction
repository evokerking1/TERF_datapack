...
execute as @e[type=marker,tag=datapipes_lib_power_generator,scores={datapipes_lib_power_storage=1..}] at @s run function datapipes_lib:power_transfer/as_generator with entity @s data.power
...
$execute if score total_amount datapipes_lib matches 1.. as @e[type=marker,tag=fluid_priority_1,tag=fluid_filter_$(id),distance=..2048] at @s run function datapipes_lib:fluid_transfer/push/find_consumer_tanks with storage datapipes_lib:temp array[0]
$execute if score total_amount datapipes_lib matches 1.. as @e[type=marker,tag=!fluid_priority_1,tag=fluid_filter_$(id),tag=datapipes_lib_current_fluid_generators,distance=..2048] at @s run function datapipes_lib:fluid_transfer/push/find_consumer_tanks with storage datapipes_lib:temp array[0]
